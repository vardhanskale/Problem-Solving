...
public class NQueens {
	...
	public static void nqueens2(int n) {
		System.out.println("In nQueens2 , n = " + n);
		int[] res = new int[n];
		auxNqueens2(0, n, res);
	}
	...
	private static boolean isValid2(int d, int c, int[] res) {
		...
		System.out.println("isValid2 : returning true");
		return true;
	}

	private static void auxNqueens2(int d, int n, int[] res) {
		// System.out.println("In auxNqueens2 : " + "depth = " + d + " res =" +
		// Arrays.toString(res) );
		if (d == n) {
			System.out.println("Reached leaf node" + Arrays.toString(res));
			return;
		}
		for (int c = 0; c < n; ++c) {
			if (isValid2(d, c, res)) {
	...
	public static void main(String[] args) {
		System.out.println("Calling nQueens2");
